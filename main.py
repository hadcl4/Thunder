...
def yt_channel():
    ...
    process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
def hadcl4_home():
    command = f''+browser+' https://hadcl4.itch.io'
    process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
def thunder_chat():
...
def pcsxr():
    if mesaon == "0":
        command = f'cd ~/Thunder/emulators/pcsxr && ./start'
    if mesaon == "1":
        command = 'LD_LIBRARY_PATH=$LD_LIBRARY_PATH:'+mesa64+':'+mesa32+' ~/Thunder/emulators/pcsxr/start'
    process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
def melonds():
    if mesaon == "0":
...
menubar = MenuBar(app,
                  ...
                  options=[
                      ...
                      [ ["ThunderCloud Data Transfer", cloud_cli], ["Steam Chat", thunderclient], ["Itch.io Community", itchchat], ["Thunder Discussions", thunder_chat], ["Hadcl4 Studios Youtube Channel",yt_channel], ["Hadcl4 Studios Itch.io Page",hadcl4_home] ],
