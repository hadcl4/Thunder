...
### ChromeOS
On ChromeOS you'll first need to activate a Linux development environment. Head to Settings -> Developers -> Linux Development Environment, then activate. ChromeOS will then download a minimal Debian installation. Before cloning Thunder, run `sudo apt update && sudo apt upgrade`. Also, because ChromeOS' Debian environment is very minimal, you'll also need to install nano (`sudo apt install nano`). After you have updated and installed nano, exit your development environment, and head back to the settings for your development environment. Adjust the size to approximately 12GB. This will give enough room for Thunder, Steam, Box86, Box64, and a several games. After you've adjusted, follow the steps listed for Debian/Ubuntu based distros.
